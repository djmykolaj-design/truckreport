...
export default function TripFuel() {
  ...
  const [reefEntries, setReefEntries] = useState(
    trip?.reefEntries || []
  );

  const [reefType, setReefType] = useState("main");
  ...
  const saveFuel = () => {
    if (tab === "reef") {
      ...
      const updatedReef = [newReef, ...reefEntries];
      setReefEntries(updatedReef);

      persistTrip({ reefEntries: updatedReef });
    ...
    const updatedFuel = [newFuel, ...fuelEntries];
  ...
  const deleteFuel = (id) => {
    if (!window.confirm("Видалити заправку?")) return;

    const updatedFuel = fuelEntries.filter((f) => f.id !== id);
    setFuelEntries(updatedFuel);
    persistTrip({ fuelEntries: updatedFuel });
  };

  const deleteReef = (id) => {
    if (!window.confirm("Видалити реф?")) return;

    const updatedReef = reefEntries.filter((r) => r.id !== id);
    setReefEntries(updatedReef);
    persistTrip({ reefEntries: updatedReef });
  };

  const addReefRefuel = (reefId) => {
    if (!reefRefuelLiters) return;

    const now = new Date().toLocaleString();

    const updatedReefs = reefEntries.map((reef) => {
      if (reef.id !== reefId) return reef;
      return {
        ...reef,
        refuels: [
          ...(reef.refuels || []),
  ...
  const deleteReefRefuel = (reefId, refuelId) => {
    ...
    const refuel = reef?.refuels?.find((r) => r.id === refuelId);
    if (!refuel) return;

    const updatedReefs = reefEntries.map((r) => {
      if (r.id !== reefId) return r;
      return {
        ...r,
        refuels: (r.refuels || []).filter((x) => x.id !== refuelId),
      };
    });
    ...
    const updatedExpenses = (trip.expenses || []).filter(
      (expense) =>
        !(
          ...
          Number(expense.amount) === Number(refuel.amount) &&
  ...
  return (
    <div style={{ color: "white", maxWidth: "1000px" }}>
      ...
      <div style={{ display: "flex", gap: "10px", marginBottom: "20px" }}>
        <button
          style={{
            ...greenButton,
            flex: 1,
            background: tab === "diesel" ? "#22c55e" : "#374151",
          }}
        ...
        <button
          style={{
            ...greenButton,
            flex: 1,
            background: tab === "reef" ? "#22c55e" : "#374151",
          }}
          onClick={() => setTab("reef")}
        >
          ❄️ Реф
      ...
      {tab === "reef" && (
        ...
        >
          ...
          <select
            ...
            style={inputStyle}
          >
            <option value="main">Основний бак</option>
            <option value="separate">Окремий бак</option>
          </select>
          <input
            type="number"
      ...
      <Card title="Всі заправки" subtitle={`${fuelEntries.length} записів`}>
        {fuelEntries.length === 0 && <p>Заправок ще немає</p>}
      ...
      {tab === "reef" && (
        <Card title="Всі рефи" subtitle={`${reefEntries.length} записів`}>
          {reefEntries.length === 0 && <p>Записів ще немає</p>}
